...
from modules.shared.geo_viz import (
    plot_geo_migration as _plot_geo_migration_shared,
    bearing_deg, GEN_COLORS,
)
...
def _render_main_content(G, df, cache, max_gen, f_threshold, families):
    ...
    def build_ancestor_geo_data(G, person_id: str, cache: dict, max_gen: int = 8):
        from modules.shared.geo_viz import build_ancestor_geo_data as _build_shared
        return _build_shared(
            G, person_id, cache, max_gen=max_gen,
            compute_inbreeding_fn=compute_inbreeding,
            ancestors_with_distance_fn=ancestors_with_distance,
        )

    def plot_geo_migration(mapped: list, show_lines: bool = True, G=None, person_id: str = None):
        return _plot_geo_migration_shared(mapped, show_lines=show_lines, G=G, person_id=person_id)
